<template>
    <div>
        ...
        <div class="total" v-if="status === 0">
            ...
            <div class="selected">
                <p>共选中{{this.checkedIds.length}}条账单</p>
                <p class="sum">合计&yen;{{totalAmount}}</p>
            </div>
...
export default {
    ...
    computed: {
        ...
        list: function() {
            var that = this
            return _.map(_.filter(that.billList, function(o) {
                return _.includes(that.checkedIds, o.Id)
            }), function(v) {
                return {
                    Id: v.Id,
                    Amount: v.Amount
        ...
        totalAmount: function() {
            return _.round(_.sumBy(this.list, 'Amount'), 2)
        }
    },
    created() {
        this.getBillList()
